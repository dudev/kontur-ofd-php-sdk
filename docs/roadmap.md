...
- ~~Интеграционный набор против тестовой площадки~~ — сделано: `tests/Integration/`, отдельный
  ...
  - CI: отдельный workflow `.github/workflows/integration.yml` — на push в `main`, на каждый PR,
    вручную (`workflow_dispatch`) и раз в неделю (ловит изменения на стороне площадки, когда
    коммитов нет); секреты `KONTUR_OFD_API_KEY`/`KONTUR_OFD_LOGIN`/`KONTUR_OFD_PASSWORD`, без них
    (PR из форков, пока секреты не заведены) job ничего не делает — notice в логе. Новый push в
    ту же ветку/PR отменяет незавершённый прогон.
